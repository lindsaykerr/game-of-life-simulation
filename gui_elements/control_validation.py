...
class ValidationMethods:
    ...
    def is_valid_x(
        ...
        if is_integer(value) and int(value) in range(kargs['start'],
                                                     kargs['end'] + 1):
    ...
    def is_Y_position(value, var, **kargs):
        return is_integer(value) and 0 <= int(value) < int(
            TkAppVariables.grid_y.get())

    @staticmethod
    def is_X_position(value, var, **kargs):
        return is_integer(value) and 0 <= int(value) < int(
            TkAppVariables.grid_x.get())


class ValidationAssignment:
    """
    Used to assign a validation method to a general method which in turn 
    can be used by tkinter for input validation.
    ...
    method(value) -- function
        accepts a tkinter variable for validation testing, this variable can
        also be modified by a validation function
    ...
    def __init__(self, input_var: tk.Variable, func, **kargs) -> None:
        ...
        func -- ValidationMethod.staticmethod
            a validation method which checks the constraints placed on the
            input_var holds true.
        **kargs
            any keyword arguments that are required by the validation
            method/function
