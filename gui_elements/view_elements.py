...
class GoFView:
    ...
    def _application_struct(self):
        ...
        frm_cvs_container = tk.Frame(self.root, width=760, height=760, bg="yellow")
        ...
        frm_cvs_container.grid  (row=1, column=7, rowspan=11, columnspan=6, sticky="NESW")
    ...
    def _grid_options(self, frame:tk.Frame):
        ...
        _x_box = self._spinbox_option(_frm_x, boxwidth=4, prefixText="X:")
        _frm_y = tk.Frame(wrapper)
        _y_box = self._spinbox_option(_frm_y, boxwidth=4, prefixText="Y:")
    ...
    def _set_pattern(self, frm, text="Place Cell Pattern"):
        ...
        _option_list = ttk.OptionMenu(frm_place, TkAppVariables.pattern_type_var, *cell_types)

        _line1 = tk.Frame(frm_place)
        _x_box  = self._spinbox_option(_line1, prefixText="X:", boxwidth=5, align=["left", "left", "left"])
        _line2 = tk.Frame(frm_place)
        _y_box  = self._spinbox_option(_line2, prefixText="Y:", boxwidth=5, align=["left", "left", "left"])
    ...
    def _sim_settings(self, frame):
        ...
        _cycles = self._spinbox_option(_row1, boxwidth=6, prefixText="Cycles:")
        _delays = self._spinbox_option(_row2, boxwidth=6, prefixText="Delay (ms):")
    ...
    @staticmethod
    def _sub_heading(a_frame, text:str, pady=5, padx=0,  headingStyle=None, separatorStyle=None):
        wrapper  = ttk.Frame(a_frame)
        _sub_head = ttk.Label(wrapper, text=text, style=headingStyle, justify="left")
        _sub_sep  = ttk.Separator(wrapper, orient="horizontal", style=separatorStyle)
    ...
    @staticmethod
    def _spinbox_option(frm, prefixText=None, boxwidth=5, posfixText=None, align=["left", "left", "left"]):
        """
        Creates a general purpose spinbox
        """
        if prefixText:
            _lbl = ttk.Label(frm,text=prefixText)
        ...
        if posfixText:
            _lbl_end = ttk.Label(frm, text=posfixText)
